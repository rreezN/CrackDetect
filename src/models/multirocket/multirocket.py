# Original implementation by: Chang Wei Tan, Angus Dempster, Christoph Bergmeir, Geoffrey I Webb
#
# MultiRocket: Multiple pooling operators and transformations for fast and effective time series classification
# https://arxiv.org/abs/2102.00457
#
# Extended to work for time series extrinsic regression tasks
...
class MultiRocket(nn.Module):
    ...
    def forward(self, X):
        ...
        xx = np.diff(X, 1)
        self.base_parameters = self.fit(
            X,
            num_features=self.num_kernels
        )
        self.diff1_parameters = self.fit(
            xx,
            num_features=self.num_kernels
        )
        x_train_transform = self.transform(
            X, xx,
    ...
    def transform(self, X, X1, parameters, parameters1, n_features_per_kernel=4):
    ...
    def fit_biases(self, X, num_channels_per_combination, channel_indices, dilations, num_features_per_dilation, quantiles):
        num_examples, num_channels, input_length = X.shape
    ...
    def fit_dilations(self, input_length, num_features, max_dilations_per_kernel):
        num_kernels = 84
    ...
    def fit(self, X, num_features=10_000, max_dilations_per_kernel=32):
        _, num_channels, input_length = X.shape
        
        num_kernels = 84

        dilations, num_features_per_dilation = self.fit_dilations(input_length, num_features, max_dilations_per_kernel)

        num_features_per_kernel = np.sum(num_features_per_dilation)

        quantiles = self.quantiles(num_kernels * num_features_per_kernel)
        ...
        biases = self.fit_biases(X, num_channels_per_combination, channel_indices,
